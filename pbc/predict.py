...
if __name__ == "__main__":
    ...
    parser.add_argument('--val_dir','-v', default='val',help="val image dir")
    parser.add_argument('--image',  '-i', default='',help="one image")
    parser.add_argument('--display','-d', action='store_true')
    parser.add_argument('--save',   '-s', default='/dev/shm',help="saved name or dir")
    parser.add_argument('--limit',  '-l', default=30, type=int)
    ...
    if os.path.isdir(args.val_dir):
        print(f'test save to {args.val_dir}')
        os.makedirs(f"{args.save}/pbctest",exist_ok=True)
        count = 0
        imgs = glob(args.val_dir + "/*.png")
        if imgs == []:
            imgs = glob(args.val_dir + "/*.jpg")
        for img in imgs:
            ...
            cv2.imwrite(f"{args.save}/pbctest/{bname}", result_img)
